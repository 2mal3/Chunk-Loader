...
function ~/clock/tick:
    ...
    execute as @e[type=minecraft:block_display,tag=chlo.chunk_loader] at @s unless block ~ ~ ~ minecraft:hopper{CustomName: '{"color":"yellow","text":"Chunk Loader"}'} run function ./loader/destroy
...
function ~/clock/10_second:
    ...
    # Ambient effects
    execute as @e[type=minecraft:block_display,tag=chlo.chunk_loader,tag=chlo.loading] at @s if entity @p[distance=..16] positioned ~0.5 ~ ~0.5 run function ./loader/ambient
